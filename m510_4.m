...
tic;
    ...
    GetMask4(maxindex,p,y_,mask,wd);
...
key1=p(17,:,:); 
key1=uint8(key1);
key1=reshape(key1,512,512);
for i=1:128;
   i1=(i-1)*4+1;
   i2=i*4;
   Ij(1:4,i1:i2)=bitxor(key1(1:4,i1:i2),Ij(1:4,i1:i2));
end

key2=p(2,:,:); 
key2=uint8(key2);
key2=reshape(key2,512,512);
for i=2:128;
   i1=(i-1)*4+1;
   i2=i*4;
   Ij(i1:i2,1:4)=bitxor(key2(i1:i2,1:4),Ij(i1:i2,1:4));
end
...
for i=1:128;
   i1=(i-1)*4+1;
   i2=i*4;
   Ij1r(1:4,i1:i2)=bitxor(key1(1:4,i1:i2),Ij(1:4,i1:i2));
end
Ijj(1:4,:)=Ij1r(1:4,:);
...
for i=2:128
    i1=(i-1)*4+1;
    i2=i*4;
    Ij1r(i1:i2,1:4)=bitxor(key2(i1:i2,1:4),Ij(i1:i2,1:4));
end
Ijj(:,1:4)=Ij1r(:,1:4);
...
for i=1:2^b
    i
    Ijrep(i,5:512,5:512)=Ij__(i,5:512,5:512);
end
% 
% Ijbit_=gather(Ijbit);
% Ijj_=gather(Ijj);
...
%找出sum11中每一块对应的秘钥序号
[qq,mk]=min(sum1,[],1);
mk=reshape(mk,128,128);
%此处转置
mk=mk';
mask_=zeros(512,512);
GetMask4(maxindex,p,mk,mask_,wd);
mmask_=mask_(5:512,5:512);
% mk=mk';
% mk=mk(:);%把mk排成一维的列向量
%A=Ijjrep(6,:,:);
